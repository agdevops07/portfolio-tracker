// ═══════════════════════════════════════════════
// DRILLDOWN
// Per-stock detail view with history + day charts.
// ═══════════════════════════════════════════════
...
export async function openDrilldown(ticker) {
  ...
  const todayChgAbs = (lp && pc) ? (lp - pc) * h.totalQty : null;
  ...
  document.getElementById('dd-cards').innerHTML = `
    <div class="stat-card">
      ...
      ${pc ? `<div class="stat-sub" style="color:${colorPnl(lp-pc)}">Prev close: ${pc.toFixed(2)}</div>` : ''}
    </div>
    <div class="stat-card">
      ...
      <div class="stat-sub" style="color:${todayChgAbs != null ? colorPnl(todayChgAbs) : 'var(--text2)'}">
        ${todayChgAbs != null ? (todayChgAbs >= 0 ? '+' : '') + fmt(todayChgAbs) : ''}
      </div>
    </div>
    <div class="stat-card">
  ...
  // Day chart – fetch if needed
  if (!state.dayHistories[ticker] || !state.dayHistories[ticker].length) {
    const dayData = await fetchDayHistory(h.ticker, h.upstoxTicker);
    state.dayHistories[ticker] = dayData;
  }
